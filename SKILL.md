...
2. **Show the dashboard, then build the rest.** Print everything above the `[AGENT INSTRUCTIONS]` line **AS-IS as a Markdown list** — each asset on its own line (don't merge into a paragraph, don't wrap in a code block). Keep the **🎯 Top setup** line. In the assets overview the dot + arrow are the **directional lean** (🟢 bullish ↑ / 🔴 bearish ↓ / 🟡 neutral), the `%` is Kronos's **conviction** in that direction (over the long then short horizon), and `~price` is its **forecast price** for that horizon. Then follow the embedded instructions to add **### 📰 Headlines** (tag each 🟢/🔴/⚪ and ALWAYS keep the color key in the header), **### ⚖️ Verdict** (bold Risk-on/off/Mixed lead + one-paragraph read grounded in the conviction, top setup, and the hidden market-context line + a Conviction badge), and **### 👉 What now?**. Never invent or alter a number from the card.
